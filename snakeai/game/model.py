...
class Snake:
    ...
    def changeDirection(self, direction: Actions):
    ...
    @snake.setter
    def snake(self, newSnake: List[Coords]):
        self._snake = newSnake.copy()
    ...
    def step(self) -> Rewards:
        ...
        if nextHead in self._snake:
            self.isGameOver = True
            return Rewards.FAILED
        # Check collision with border
        if not (0 <= nextHead.x < self.dim and 0 <= nextHead.y < self.dim):
            self.isGameOver = True
            return Rewards.FAILED
